...
| future-scholars-website | `studio/src/schemaTypes/documents/page.ts` | `masthead` page-level field (build plan D12); `postsArchive`/`projectsArchive`/`authorsArchive` removed from `pageBuilder.of`; `archiveField` spread with `hidden: true` rather than edited in `shared.ts` | keep the FSMA field list; take template changes to the surrounding validation/options. Re-adding archives means restoring the three `of` entries and dropping the `hidden` override |
...
| future-scholars-website | `frontend/app/globals.css` | FSMA palette in `:root` and `.dark`; `--brand-accent{,-foreground,-strong}` added and exposed in `@theme inline`; `--radius` 0.75rem; `h1`–`h4` bound to `--font-display` in `@layer base`; FSMA view-transition anchors inside the view-transition block (`site-rail`, `site-topbar`, `social-rail`) | keep the FSMA token values **and** the FSMA anchor rules; take template changes to the rest of the view-transition block and utilities. A template palette change is **not** taken |
...
| future-scholars-website | `frontend/app/components/layout/{SideNav,SideNavLinks,SideNavMobile,NavContact,BrandMark,Masthead,SocialRail}.tsx` (new) | FSMA app shell (S4) and masthead/social chrome (S5) | new files, no conflict |
