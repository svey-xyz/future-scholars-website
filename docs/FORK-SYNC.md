...
| future-scholars-website | `studio/src/schemaTypes/objects/testimonials.ts` | `source` (manual \| documents), `featuredOnly`, `limit`, `layout` (cards \| letters; `columns` hidden for letters); the inline array is hidden and its `required` relaxed when sourcing from documents | keep the source switch; the template's inline array is untouched underneath it |
...
| future-scholars-website | `frontend/app/components/blocks/Testimonials.tsx`, `TestimonialAttribution.tsx` (new), `TestimonialLetters.tsx` (new) | normalises the template's inline array and FSMA's `testimonial` documents to one shape; `limit` applied in JS (S6); `layout: 'letters'` dispatches to the full-page letters renderer; the card's avatar/name/source row is extracted to `TestimonialAttribution` | keep the normalisation and the layout switch; port template card-markup changes into the card branch / `TestimonialAttribution` |
...
| future-scholars-website | `frontend/app/programs/[slug]/page.tsx` (new), `studio/sanity.config.ts`, `frontend/app/sitemap.ts`, `frontend/sanity/lib/queries.ts` | `program` detail route (S8): `programQuery`/`programSlugsQuery`, `program` in `sitemapData` + sitemap switch, Presentation `mainDocuments`/`locations`/`resolveHref` | FSMA-only type; conflicts are the one extra line in each shared list/switch — keep them |
| future-scholars-website | `frontend/app/components/common/Breadcrumbs.tsx` (new), `frontend/app/components/seo/JsonLd.tsx` (`breadcrumbJsonLd`) | visual breadcrumb + `BreadcrumbList` from one item list (S8) | **backport candidate** — generic, new files/exports only |
