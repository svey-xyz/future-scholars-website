...
| future-scholars-website | `frontend/lib/utils.ts`, `frontend/app/components/layout/NavContact.tsx` | `telHref()` helper; the rail's phone link uses it instead of emitting a country-code-less `tel:` (S7) | **backport candidate** — the template's `NavContact` has the same bug. Keep the FSMA fix until upstream has its own |
