...
| future-scholars-website | `frontend/app/globals.css` | FSMA palette in `:root` and `.dark`; `--brand-accent{,-foreground,-strong}` added and exposed in `@theme inline`; `--radius` 0.75rem; `h1`–`h4` bound to `--font-display` in `@layer base` | keep the FSMA token values; take template changes to the non-token layers (view-transition block, utilities). A template palette change is **not** taken |
