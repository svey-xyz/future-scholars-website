...
## Phase 8 — CMS-driven header navigation — DONE (host-side verify pending)

Replaces the hard-coded single-`navLinks`-array header (Phase 4) with a Sanity-driven nav: `settings.navigation` is an array of `navLink` (leaf, reuses the `link` object) and `navDropdown` (a labelled disclosure that groups links). Studio-side: `navLink`/`navDropdown` objects registered in `schemaTypes/index.ts`; `settings.tsx` gains `navigation`, plus a `mobileNav.showFooterContent` toggle and `legal`. Query: `settingsQuery` projects the nav tree (`navLinkProjection` resolves `page->`/`post->`/`href` and a `resolvedTitle` coalesce).

New primitive:

- **`collapsible`** added to `frontend/components/ui/` — thin re-export of `@radix-ui/react-collapsible` (`Root` / `CollapsibleTrigger` / `CollapsibleContent`), no icon swap needed (Radix Collapsible ships none). Drives the mobile dropdown disclosures. The `collapsible-down` / `collapsible-up` height keyframes come from `tw-animate-css` (already imported), so no `globals.css` addition was required.

New components:

- **`HeaderNav`** (`'use client'`) — owns the desktop-inline ↔ collapsed-hamburger decision by *available space*, not a fixed breakpoint. Measures the nav track's `clientWidth` vs the nav's intrinsic `scrollWidth` via `useLayoutEffect` + a debounced `ResizeObserver`; collapses when intrinsic > available. The inline nav stays mounted when collapsed (so widening re-measures and re-expands) but is `inert` + `aria-hidden`. Pre-hydration CSS fallback (`hidden md:flex` / `md:hidden`) keeps first paint usable with no JS.
- **`DesktopNav`** (`'use client'`) — presentation-only Radix/shadcn `NavigationMenu`. `navLink` → leaf via `ResolvedLink`; `navDropdown` → a `NavigationMenuTrigger` (`<button>`, never navigates) + `NavigationMenuContent` panel. Active leaf gets `aria-current="page"` (set explicitly — Radix's `data-active` doesn't survive the `asChild` → `ResolvedLink` boundary), which the `aria-[current=page]:…` styling keys off; external links are never marked active.
- **`MobileNav`** (`'use client'`) — the `Sheet` (Radix Dialog) drawer. Hamburger is an icon-only `Button` with `aria-label` + `aria-expanded` reflecting open state; three-bar → X morph is CSS, motion-safe. Dropdowns use `Collapsible`; the section auto-opens if it contains the active route. Mount-gated via `useSyncExternalStore` (same React-19 SheetTrigger hydration dodge as the old `MobileMenu`). Optionally renders `FooterContent` when `mobileNav.showFooterContent`.
- **`FooterContent`** (RSC) — shared footer body (socials + legal disclaimer) rendered by both the site `Footer` and the mobile `Sheet`, so the two never drift. Socials are external-safe links (`target="_blank"` + `rel` + `sr-only` "(opens in new tab)").

Changed:

- **`ResolvedLink`** is now `forwardRef` (`forwardRef<HTMLAnchorElement, …>`) and spreads remaining props, so it composes cleanly with Radix `asChild`/`Slot` — the ref, `onClick`, and parent-injected `data-*`/`aria-*` (e.g. `NavigationMenuLink`'s) all reach the underlying anchor. Convenience `ariaCurrent`/`ariaLabel` aliases added.
- **`Header`** now fetches `settings.{navigation,mobileNav,contact,legal}` and renders `<HeaderNav>` (replacing the inline `NavigationMenu` + `MobileMenu`). **`Footer`** delegates its socials/legal row to `<FooterContent>`.
- **`MobileMenu`** reduced to a `@deprecated` re-export of `MobileNav` (filesystem unlink is blocked on the sandbox mount, so the old module is left as a thin shim rather than deleted — see RUN THIS NEXT).

A11y patterns for the nav are codified in [docs/A11Y.md](./A11Y.md) (Navigation section): disclosure-only dropdown buttons, `aria-current` active leaf, hamburger label + state, external-link cue, Sheet focus-return, `inert` collapsed nav. Lint + type-check + TypeGen ran clean in-sandbox; `next build`, Lighthouse, and live keyboard/SR sweep remain host-only (Sanity/Google-Fonts network is unavailable in the sandbox).

**RUN THIS NEXT (host):** `git rm frontend/app/components/MobileMenu.tsx` once you confirm no external import depends on the shim, then run the host build + Lighthouse (light + dark, ≥95) and a keyboard/VoiceOver sweep of the header at both inline and collapsed widths (resize to confirm the available-space collapse threshold).
...
- **Added in phase 8**: `frontend/components/ui/collapsible.tsx`; `frontend/app/components/{HeaderNav,DesktopNav,MobileNav,FooterContent}.tsx`; `studio/src/schemaTypes/objects/{navLink,navDropdown}.ts`.
- **Edited in phase 8**: `frontend/app/components/{Header,Footer,ResolvedLink,MobileMenu}.tsx` (`ResolvedLink` now `forwardRef`; `MobileMenu` now a `@deprecated` re-export of `MobileNav`), `frontend/sanity/lib/{queries,types}.ts`, `studio/src/schemaTypes/index.ts`, `studio/src/schemaTypes/singletons/settings.tsx`, `frontend/sanity.types.ts` + `sanity.schema.json` (TypeGen-regenerated — do not hand-edit).
- **Untouched through phase 7** (phase 8 since touched `sanity/lib/`, `studio/`, and the generated type/schema files — see above): `frontend/sanity/**`, `studio/**`, `sanity.types.ts`, `sanity.schema.json`.
