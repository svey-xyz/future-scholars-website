...
**Status:** S1–S5 done · content + images seeded · S0b outstanding (owner; **now blocked on Q20**) · **Last updated:** 2026-09-13 (rev 10) · **Owner:** Hayden Soule (svey)
...
| Q20 | `next build` fails under Cache Components when a hidden content type has zero documents: `/posts/[slug]` and `/projects/[slug]` `generateStaticParams` return `[]`, and Next 16 requires ≥1 param (`empty-generate-static-params`). Pre-existing fork condition (D13 hides the types; the dataset has no posts/projects) — **blocks the first Vercel deploy (S0b)**. Fix direction: return a placeholder slug that renders `notFound()` when the list is empty — a template-level fix, so **backport candidate** (§0 rule 6) | svey | **Opened 2026-09-13 (S5)** — found by the first on-Mac production build, which compiled and type-checked cleanly and failed only at page-data collection |
...
| 2026-09-13 | S5 | opencode/kimi | `feat/fsma-s5-masthead-socials` | **Done, one item carried.** Masthead (both variants) + floating social rail built and wired through `CachedPage`; masthead owns the visual h1; rail-footer socials deduped (§12); `masthead` added to `getPageQuery`, typegen regenerated; type-check/lint/format clean. **First session run on the owner's Mac** (OpenCode), not the Cowork VM — §5.1's sandbox quirks didn't apply: typegen ran as one script, the owner's dev server rendered all six routes 200, mastheads verified in HTML (per-page height/tone/placement, exactly one h1 per route). Image-weight/LCP measurement carried (no photography; Q20 blocks builds) | **New Q20**: `next build` fails on `/posts/[slug]` + `/projects/[slug]` — empty `generateStaticParams` under Cache Components (pre-existing, blocks S0b's first deploy). Q18/Q19 are Cowork-VM-only and did not reproduce on the Mac: the production build compiled and type-checked cleanly before hitting Q20 |
