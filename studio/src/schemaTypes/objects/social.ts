...
export const social = defineType({
  ...
  fields: [
    defineField({
      ...
      type: 'string',
      description:
        'The title associated with the account prefixed with the address sign (e.g. @username)',
  ...
  preview: {
    ...
    prepare(value: any) {
      const socialSitePlatformTitle =
        value.type &&
        platforms.flatMap((option) => (option.value === value.type ? [option.title] : []))
